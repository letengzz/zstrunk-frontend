  ...
  <div class="products-page-container">
    ...
    <div class="products-main-section">
      ...
      <div class="category-specs" v-if="currentCategory !== 'all'">
        <h3 class="specs-title">Parameter</h3>
        <table class="specs-table">
          <tbody>
            <tr v-for="(value, key) in categorySpecs.params" :key="key">
      ...
      <div class="category-types" v-if="categoryTypes.items.length > 0">
        <h3 class="types-title">Types of {{ findCategoryLabel(categoryTree, currentCategory) }}</h3>
        <p class="types-intro">{{ categoryTypes.introText }}</p>
        <div class="types-grid">
          <div class="type-card" v-for="(type, index) in categoryTypes.items" :key="index">
...
const categorySpecsMap: Record<string, CategorySpecsData> = categorySpecsData as Record<string, CategorySpecsData>
...
const categorySpecs = computed(() => {
  if (currentCategory.value === 'all') {
    return { feature: '', params: {}, types: undefined }
  }
  return categorySpecsMap[currentCategory.value] || { feature: '', params: {}, types: undefined }
})

const categoryTypes = computed(() => {
  const specs = categorySpecs.value
  if (!specs.types) {
